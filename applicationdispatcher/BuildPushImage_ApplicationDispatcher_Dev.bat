...
docker build -f PDI.ApplicationDispatcher.BlazorServer/Dockerfile.dev -t tczaplic/pdi:applicationdispatcherblazorserver .

docker images

docker image ls --filter label=stage=applicationdispatcherblazorserver_build

::docker image prune --filter label=stage=applicationdispatcherblazorserver_build
